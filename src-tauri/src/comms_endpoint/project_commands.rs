...
pub async fn get_all_projects()->Result<String, String>{
    let projects = project_service::get_all_projects().await
        .map_err(|err|serialize_error(format!("[Project Command - GEt all project failed]: {:?}", err)))?;
...
pub async fn get_project_deployment(project_name:&str, deploy_name:&str) -> Result<String, String>{
    let deployment = project_service::get_project_deployment(project_name, deploy_name).await
        .map_err(|err|serialize_error(err))?;
