...
pub async fn get_data_for_class(bucket_name:&str, path:&str, page:&str)->Result<ImageListResult, AWSClientError>{
    ...
    let res = bucket_client(bucket_name).unwrap().list_page(path.to_owned(),Some("".to_string()), continuation_token.clone(), None, Some(10)).await
        .map_err(|err| AWSClientError::ObjectRetrievalError(err.to_string()))?;
...
pub async fn delete_object(bucket_name:&str, file_path:&str)->Result<(), AWSClientError>{
    match bucket_client(bucket_name).unwrap().delete_object(file_path.to_string()).await{
        Ok(_)=>Ok(()),
