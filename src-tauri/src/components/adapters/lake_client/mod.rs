...
impl LakeClient {
    ...
    pub fn new(bucket_name: &str) -> Result<LakeClient, LakeClientError> {
        ...
        match Credentials::default() {
            Ok(creds) => Ok(LakeClient {
                client: *Bucket::new(&bucket_name, region, creds).unwrap(),
            }),
            Err(err) => Err(LakeClientError::InitClientError(format!(
    ...
    pub async fn get_data_for_class(
        ...
        path: &str,
        page: &str,
    ) -> Result<ImageListResult, LakeClientError> {
    ...
    pub async fn sync_repo(self, base_path: &str) -> Result<Vec<u8>, LakeClientError> {
        ...
        match &self.get_sync_file(file_name).await {
            ...
            Err(err) => {
                ...
                let res = &self
                    .client
                    .list(base_path.to_owned(), Some("/".to_string()))
                    .await
                    .unwrap();
    ...
    pub async fn create_sync_file(
        ...
        self.client
            .put_object(file_name, text_lines)
            .await
            .map_err(|err| LakeClientError::ObjectRetrievalError(err.to_string()))?;
