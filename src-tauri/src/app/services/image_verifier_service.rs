...
use crate::app::services::data_lake_service::{
    delete_data_for_class, get_data_for_class, list_all_classes,
};
use ml_cleaner::client_adapters::image_verifier::{
    ImageVerifiedRecord, ImageVerifierClient, ImageVerifierError, Pagination,
};
...
pub async fn sync(project_name: &str) -> Result<(), ImageStoreServiceError> {
    ...
    for c in list_all_classes(project_name)
        .await
        .map_err(|err| ImageStoreServiceError(err.to_string()))?
    {
        let class_id = match verifier_client.get_class_id_by_name(&c).await {
            ...
            Err(ImageVerifierError::ObjectNotFound) => {
                println!("'{}' not found in database", c);
                None
            }
            Err(v) => return Err(ImageStoreServiceError(v.to_string())),
        };
        ...
        let mut class_data = get_data_for_class(project_name, &c, "")
            .await
            .map_err(|err| ImageStoreServiceError(err.to_string()))?;
        ...
        loop {
            ...
            for im in &images {
                match verifier_client
                    .insert_image_verification(&c, &im.file_path, false)
                    .await
                {
                    Ok(_) => inserted += 1,
            ...
            println!(
                "Successfully inserted '{}' image(s) to verified_images. Found '{}' duplicates",
                inserted, dup_count
            );
            ...
            class_data = get_data_for_class(project_name, &c, &page)
                .await
                .map_err(|err| ImageStoreServiceError(err.to_string()))?;
...
pub async fn get_unverified_images_for_class(
    ...
    class_name: &str,
    page: Option<&str>,
) -> Result<ImageCollection, ImageStoreServiceError> {
    let mut verifier_client = ImageVerifierClient::new().await.unwrap();

    let image_paths = verifier_client
        .get_unverified_images_for_class(class_name, page)
        .await
        .map_err(|err| ImageStoreServiceError(err.to_string()))?;
    ...
    for p in image_paths.0 {
        let tmp = data_lake_service::get_data_by_path(project_name, &p)
            .await
            .map_err(|err| ImageStoreServiceError(err.to_string()))?;
...
pub async fn verify_image(file_path: &str) -> Result<(), ImageStoreServiceError> {
    ...
    verifier_client
        .update_image_verification_status(file_path, true)
        .await
        .map_err(|err| ImageStoreServiceError(err.to_string()))?;
...
pub async fn remove_image(
    ...
    delete_data_for_class(project_name, file_path)
        .await
        .map_err(|err| ImageStoreServiceError(err.to_string()))?;
...
pub async fn list_all_images_by_class(
) -> Result<HashMap<String, Vec<String>>, ImageStoreServiceError> {
    let mut verifier_client = ImageVerifierClient::new().await.unwrap();
